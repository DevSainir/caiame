"""
The programme of a course: its modules, the lectures inside them and the material attached.

Split from `administration` when that file reached the size where a reader stops holding it
in their head: courses are one subject — what a course is, whether it is published — and the
outline inside a course is another. They meet only through the identifier of the course.

Every method starts from the course, not from the id in the address: a module that belongs
to another course is «not found» here, so a guessed identifier cannot edit somebody else's
programme.
"""
...
from models.enums import CourseUnitKind, MediaStatus
...
class CourseNotFoundForProgrammeError(Exception):
...
class ProgrammeStore(Protocol):
    """What editing an outline needs from storage."""
...
class ProgrammeService:
    """Building the outline of one course and hanging material on its lectures."""

    def __init__(
        self,
        *,
        admin_repo: ProgrammeStore,
        unit_repo: UnitStore,
    ...
    async def _course(self, course_id: UUID) -> Course:
        ...
        if course is None:
            raise CourseNotFoundForProgrammeError(course_id)
        return course
