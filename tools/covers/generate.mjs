...
const SEED = JSON.parse(
  readFileSync(resolve(ROOT, "backend/scripts/seed_data.json"), "utf8"),
);
...
const SCHEMES = [
  {
    from: color("primary", 50),
    to: color("primary", 200),
    ink: color("primary", 500),
    alpha: 0.5,
  },
  {
    from: color("primary", 500),
    to: color("primary", 800),
    ink: "#ffffff",
    alpha: 0.18,
  },
  {
    from: color("primary", 900),
    to: color("primary", 950),
    ink: color("primary", 400),
    alpha: 0.3,
  },
  {
    from: color("primary", 400),
    to: color("primary", 600),
    ink: "#ffffff",
    alpha: 0.2,
  },
  {
    from: color("primary", 950),
    to: color("primary", 700),
    ink: color("success", 500),
    alpha: 0.35,
  },
];
...
// Only for courses without a photograph: `cover` in the seed file points at a real image
// for everything the academy has shot, and at the card this script draws for the rest.
// A generated card silently overwriting a photo would be noticed weeks later.
const COVERS = SEED.courses
  .filter((course) => String(course.cover).endsWith(".svg"))
  .map((course, index) => ({
    slug: course.slug,
